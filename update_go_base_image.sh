...
current=$(sed -nE 's|^FROM (--platform=[^ ]+ )?docker\.io/library/golang:([0-9]+(\.[0-9]+)*)-.* AS build$|\2|p' "$dockerfile")
...
variant=$(sed -nE 's|^FROM (--platform=[^ ]+ )?docker\.io/library/golang:[0-9]+(\.[0-9]+)*-(alpine[0-9.]*).* AS build$|\3|p' "$dockerfile")
if [ -z "$variant" ]; then
...
# Update every golang stage (build and the target-platform tini stage) so their
# pinned digests can never drift apart.
sed -i -E "s#(^FROM (--platform=[^ ]+ )?docker\.io/library/golang:)[^ ]+ (AS (build|tini))#\1${goversion}-${variant}@${digest} \3#" "$dockerfile"
